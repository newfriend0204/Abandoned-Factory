...
public class MonologueTriggerZone : MonoBehaviour {
    ...
    public List<LocalizedString> randomMessages = new List<LocalizedString>();
    ...
    private string GetMessageToUse() {
        if (randomMessages != null && randomMessages.Count > 0) {
            ...
            for (int i = 0; i < randomMessages.Count; i++) {
                if (!string.IsNullOrEmpty(randomMessages[i].GetLocalizedString())) {
                    candidates.Add(randomMessages[i].GetLocalizedString());
                }
            }
