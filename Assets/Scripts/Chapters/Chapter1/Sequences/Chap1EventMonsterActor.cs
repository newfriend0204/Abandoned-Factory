...
public class Chap1EventMonsterActor : MonoBehaviour {
    public Transform Root => monsterRoot != null ? monsterRoot : transform;
    ...
    public IEnumerator CoWalkToPoint(Transform destination, bool snapOnArrive = false) {
        if (destination == null)
            yield break;
        ...
        yield return StartCoroutine(CoMoveTo(destination.position, approachWalkSpeed, 0f, walkNormMax, false, snapOnArrive));
    ...
    public IEnumerator CoRunToPoint(Transform destination, float speed, bool snapOnArrive = false) {
        if (destination == null)
            yield break;
        if (navAgent == null || monsterRoot == null)
            yield break;
        ...
        if (moveSpeed <= 0f)
            moveSpeed = slamRunSpeed;

        StopChargeSfx();
        SetFootstepsEnabled(true);
        ForceLocomotion();

        yield return StartCoroutine(CoMoveTo(destination.position, moveSpeed, runNormMin, 999f, false, snapOnArrive));
